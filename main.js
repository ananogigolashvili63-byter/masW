...
function renderTextbooks() {
      ...
      <div class="textbook-card slide-up" style="animation-delay: ${i * 0.07}s" data-id="${book.id}">
        ...
        <div class="textbook-info">
          ...
          <div class="textbook-actions">
            <button class="textbook-btn textbook-btn--primary" data-action="read" data-id="${book.id}">Read Online</button>
            <button class="textbook-btn textbook-btn--secondary" data-action="download" data-id="${book.id}">Download</button>
          </div>
        </div>
      </div>
    `;
  ...
  grid.querySelectorAll('.textbook-btn').forEach(btn => {
    btn.addEventListener('click', () => {
      const book = TEXTBOOKS.find(b => b.id === parseInt(btn.dataset.id));
      const action = btn.dataset.action;
      showToast(
        action === 'read'
          ? `Opening "${book.title}" in reader...`
          : `Downloading "${book.title}.pdf"...`,
        'info'
      );
    });
  });
